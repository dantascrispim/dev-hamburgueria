...
const addreWarn = document.getElementById("address-worn");
...
function addTocart(name, price) {
    ...
    if(existingItem) {
        ...
        // se o item já existe acrecentar apenas a quantidade + 1
...
// funcão para remover item do carrinho
    ...
    addressInput.addEventListener("input", function(event) {
        ...
        if(inputValue !== "") {
            ...
            addreWarn.classList.add("hidden");
        }
    ...
    checkoutBtn.addEventListener("click", function(){
        ...
        //Enviar Pedido para API do Whatssap

        const cartItems = cart.map((item) => {
            return (
                ` ${item.name} Quantidade: (${item.quantity}) Preço: RS${item.price} |`
            )
        }).join("");
